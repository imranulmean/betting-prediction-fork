contract Prediction is Ownable, Pausable, ReentrancyGuard {
    ...
    event PredictionsBet(address indexed sender, uint256 indexed currentEpoch, uint256 amount,  uint256 stakingAmount, uint8 position);
    ...
    function betBear(uint256 epoch, address user, uint256 amount) external whenNotPaused nonReentrant onlyOwner {
        ...
        //round treasury amount includes the staking and bonuses until the calculation
        ...
        emit PredictionsBet(user, epoch, amount, stakingAmt, uint8(Position.Bear));
    }
    ...
    function betBull(uint256 epoch, address user, uint256 amount) external whenNotPaused nonReentrant onlyOwner {
        ...
        emit PredictionsBet(user, epoch, amount,  stakingAmt, uint8(Position.Bull));
    }
    ...
    function claim(address user, uint256[] calldata epochs) external nonReentrant onlyOwner {
        ...
        for (uint256 i = 0; i < epochs.length; i++) {
            ...
            if (rounds[epochs[i]].oracleCalled) {
                ...
                addedReward = betInfo.amount * round.rewardAmount / round.rewardBaseCalAmount + betInfo.stakingAmount;
    ...
    function setStakingLevelBonuses(uint256[] calldata _bonuses) external onlyOwner {
        ...
        require(_bonuses[_bonuses.length - 1] <= 100, "<100");
